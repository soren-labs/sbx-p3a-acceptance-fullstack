...
Start the frontend from `frontend/` with `npm install` and `npm run dev`. Set `VITE_API_BASE_URL` to the backend URL when it is not `http://localhost:8000` (for example `VITE_API_BASE_URL=http://localhost:8000 npm run dev`). Run frontend checks with `npm run typecheck` and `npm run build` from `frontend/`.
